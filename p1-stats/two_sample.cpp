...
vector<double> mean_diff_sampling_distribution(
  ...
  for (int i = 0; i < 1000; ++i){
    value.push_back(mean(bootstrap_resample(data_A, i)) - mean(bootstrap_resample(data_B, i)));
  }
  return value;
}
...
pair<double, double> confidence_interval(vector<double> v, double width) {
  width = width/2;
  pair<double,double> bounds (double boundsLower, double boundsHigher);
  double boundsLower = (percentile(v, .50 - width)); //creating a range between width centered at 50%
  double boundsHigher = (percentile(v, .50 + width));
  return {boundsLower, boundsHigher};
}
